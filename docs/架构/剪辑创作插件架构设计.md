...
> 状态：首期核心闭环已实现；剪辑引擎能力扩展见 §18.3  
> 设计基线：`cbe62875b933ec455f719bbb5f125c2ac174a179`  
> 本文定义 Plugin、MCP Server、rules、skills、视频分析、知识存储、分阶段创作与现有剪辑引擎的最终边界。

## 1. 系统目标

本项目交付名为 `huayang` 的本地剪辑创作 Plugin，使 Codex、Claude Code 等宿主 Agent 通过同一组阶段规则、思维技能和 MCP 工具完成三类任务：
...
源码目录本身是项目插件根。构建 `huayang` Python wheel 时，同一份 `.codex-plugin`、`.claude-plugin`、`.mcp.json`、rules、skills 与 schemas 通过 wheel data files 安装到 `share/huayang`，后台静态资源通过 `video_create_plugin.admin` package data 分发；`ContextCatalog` 根据已安装 distribution 的 RECORD 定位该根目录。Codex 本地安装由 `build_codex_marketplace` 只复制宿主清单、MCP 清单、Rules、Skills 与 Schemas，不把源码、虚拟环境或产物目录带入 Marketplace。内部 Python 包保持为 `video_create_plugin`。项目插件与 wheel 共用 `huayang-mcp` console entry point，不维护第二套实现。
...
`huayang` console entry point 启动只监听 `127.0.0.1` 的后台管理服务并打开浏览器。后台通过应用服务维护 Rules 与 Skills，通过显式注册的工作流数据源和受限产物目录提供只读产物浏览；它不绕过工作流状态、冻结和发布门禁。内置 Rule/Skill 可编辑且保留删除保护，自定义资源支持新增、编辑和删除。资源更新携带内容 SHA-256 并原子落盘。
...
flowchart TD
    ...
    B["本地浏览器"] --> AM["Huayang Admin API"]
    A --> R["Main rule + Router skill"]
    R --> M["MCP Server"]
    M --> C["Context Catalog"]
...
- Admin API 只通过资源与工作流应用服务读写允许范围，产物浏览保持只读。
...
```text
huayang://catalog
huayang://rules/main-agent
huayang://rules/{role_id}
huayang://skills/{skill_id}
huayang://tasks/{task_id}/stage-envelope
huayang://stage-access/{handle}/artifacts/{artifact_id}
huayang://stage-access/{handle}/evidence/{analysis_id}/{evidence_id}
huayang://schemas/{schema_name}
```
...
```json
{
  ...
  "role_resource": "huayang://rules/creation/editing-spec-agent",
  "skill_resources": [
    "huayang://skills/editing-specification"
  ],
  "allowed_resources": [
    "huayang://stage-access/opaque_server_handle/artifacts/artifact_plan",
    "huayang://stage-access/opaque_server_handle/artifacts/artifact_assets",
    "huayang://stage-access/opaque_server_handle/artifacts/artifact_bgm"
  ],
  "allowed_tools": [
    "knowledge_search",
  ...
  "output_contract": "huayang://schemas/editing-specification",
...
1. 调用 `reference_get_creation_context`，直接取得本任务确认版参考报告的 ArtifactRef、FreezeRef 和报告内容哈希。
2. 使用工具返回的当前阶段 `creation_context_projection` 与规范化哈希。
3. 检索共享知识索引中的其他已确认案例。

本次参考分析不绕行共享向量召回，避免被历史相似案例稀释。阶段一的 `CreativeDirection`、阶段二的 `PreparationPackage` 和阶段三的 `EditingSpecification` 均写入同一 `ReferenceContextBinding` 结构；服务端按当前冻结输入重新计算并逐字段校验。原创任务不携带该绑定，但三个创作阶段同样可以检索共享知识。
...
参考驱动任务同时记录当前报告 ArtifactRef、FreezeRef、报告内容哈希、`creative_direction` 投影名称及投影哈希。
...
参考驱动任务的主汇总 Artifact 同时绑定 `resource_preparation` 投影；原创任务保持该字段为空。
...
参考驱动任务的权威规格同时绑定 `editing_specification` 投影；服务端还会验证规格中的素材、BGM、拍点和源区间均属于阶段二冻结资源包。
...
渲染成功后，`editor_inspect_render` 从冻结规格和 SpecTraceMap 唯一派生验收预期，并写入带 HMAC 的持久检查绑定。绑定覆盖任务、规格 Artifact/Freeze、编译工程、渲染快照、TraceMap、RenderJob、成片、验收预期、检查报告和联系表的身份与哈希。最终 `ExecutionManifest` 必须引用该 RenderJob 及其精确绑定路径；提交时重新读取真实检查报告并验证全部检查项通过，单独拼装同哈希副本或改写绑定均不形成有效执行产物。
...
同一源媒体的新确认 revision 发布时，在一个事务中创建新 active publication，并把该源媒体的被替代版本标记为 `superseded`。源媒体身份使用 `source_media_sha256`，报告 Artifact ID 变化不影响替代关系。历史记录继续可审计；默认检索只读取 active publication。撤回需要显式操作并保留记录。
...
- 已生成并通过完整报告模型校验的 `creation_context_projection`；
- 每条知识单元的证据引用均属于该报告的证据闭包；
- 每条 `creation_knowledge` 的知识类型与正文均来自其适用阶段的投影；
...
| `reference_get_creation_context` | 为参考驱动创作返回当前冻结报告、冻结记录、阶段投影及规范化哈希 |
...
| `editor_preflight_spec` | 读取阶段三已批准冻结的 ActionSpec，对照 Capability Registry 预检 |
| `editor_compile_spec` | 从同一冻结规格确定性编译 EditorProject 与 SpecTraceMap，并记录任务绑定 |
| `editor_validate_execution_project` | 按冻结规格校验编译快照、SpecTraceMap 与全部 ActionSpec 覆盖 |
| `editor_submit_render` | 校验任务绑定后提交工程与追溯表的不可变快照 |
| `editor_get_render` | 查询绑定当前任务冻结规格的持久化渲染任务 |
| `editor_inspect_render` | 从冻结规格和追溯表派生验收预期，检测黑帧、越界、音画长度、响度和规格覆盖，并持久化 HMAC 检查绑定 |
...
### 18.1 第一阶段：Codex 参考学习纵向闭环

交付：

- 一个已验证宿主：Codex manifest、`.mcp.json` 与 stdio MCP Server。
- Context Catalog、main rule、任务 Router、Host Adapter 与 StageEnvelope。
- reference_study 所需最小 TaskRun、StageRun、Artifact、FreezeRecord、lineage 和文件对象库。
- `video_analysis`、`audio_analysis` 与 EvidenceBundle。
- JSON + Markdown 参考报告；DOCX 版式后置。
- 用户确认后的最小 KnowledgePublication、LanceDB 发布与阶段过滤检索。

验收：

- 本地文件和下载视频均可进入分析。
- 快切区间支持不大于 0.1 秒的最大采样间隔，并使用真实 PTS。
- 报告包含总体机制、BGM、多维逐镜表和可迁移剪辑规律。
- 主镜头区间覆盖全片且无非预期空洞；镜内事件允许重叠并明确时间与作用域。
- 全部重要结论具有 evidence refs，用户确认前不进入共享知识索引。
- 当前任务直接读取本次参考 Artifact；创作知识检索只返回 active、reusable 单元。
- `audit_only` 与 `host_verified` 两种确认保证等级在记录中可区分。

### 18.2 第二阶段：创作规划与完整宿主包装

交付：

- `original_creation` 与 `reference_guided_creation`。
- 三次用户确认、依赖闭包与只读上游引用。
- 图片素材获取、媒体预处理、BGM 获取与统一溯源。
- CreativeDirection、MaterialPackage、BgmPackage、EditingSpecification + ActionSpec。
- Capability Preflight、确定性编译与 `EditorProject + SpecTraceMap`。
- Claude Code manifest 与第二宿主冒烟。
- DOCX 报告生成与版面检查。
...
- 阶段一不出现具体素材和执行参数。
- 阶段二不设计最终时间线。
- 阶段三只使用已确认素材和 BGM，并为每个执行动作分配稳定 `action_id`。
- stale 阶段的提交、冻结、发布、编译和渲染均被服务器拦截。
- CapabilityGap 出现时只生成缺口报告，工程编译尚未开始。
- 参考驱动创作使用剪辑规律，不复制参考片的具体素材与时间线。
### 18.3 第三阶段：引擎能力扩展与成片闭环
...
3. 遮罩与裁切动画。
4. 转场与效果作用域。
5. 音量包络、淡入淡出与 ducking。
6. 跨镜头延续与 Composition。
7. 预览帧与导出帧一致性检查。

每项能力同时覆盖领域模型、命令、预览、FFmpeg 编译、Capability Registry 和一致性测试。
...
uv run ruff check .
uv run mypy
MCP Inspector：resources/list、resources/read、tools/list、tools/call
Plugin 宿主冒烟：Codex、Claude Code 各完成一次 reference_study
```
